SELECT mn.name, m.name, m.inStock, q.count, p.value, p.max_discount
FROM models m
...
SELECT c.name, SUM(mn.capitalization)
FROM manufacturers mn
    JOIN countries c ON c.id = mn.country_id
GROUP BY c.name
...
SELECT c
.name, SUM
(mn.capitalization) FROM manufacturers mn
	JOIN countries c ON c.id = mn.country_id
GROUP BY c.name
HAVING SUM
(mn.capitalization) =
(SELECT SUM(manufacturers.capitalization)
FROM manufacturers
...
SELECT m.name, s.first_name, s.last_name, o.name
FROM models m
    JOIN sellers s ON s.id = m.seller_id
    JOIN offices o ON o.id = s.office_id
...
SELECT SUM(p.value * q.count) / SUM(q.count)
FROM models m
    JOIN quantity q ON q.model_id = m.id
    JOIN prices p ON p.id = m.price_id
    JOIN manufacturers mn ON mn.id = m.manufacturer_id
