...
-- Вывести всю информацию
SELECT pr.name, mn.name, mn.location, pc.value, pc.discount
FROM products pr
    JOIN manufacturers mn ON mn.id = pr.manufacturer_id
    JOIN prices pc ON pc.product_id = pr.id

-- RIGHT/LEFT JOIN
SELECT pr.name, mn.name, mn.location
FROM manufacturers mn LEFT JOIN products pr ON mn.id = pr.manufacturer_id

SELECT pr.name, mn.name, mn.location
FROM products pr RIGHT JOIN manufacturers mn ON mn.id = pr.manufacturer_id

SELECT pr.name, pc.value, pc.discount
FROM products pr LEFT JOIN prices pc ON pc.product_id = pr.id
...
SELECT mn.name, ROUND(AVG(pc.value), 0)
FROM products pr
    JOIN manufacturers mn ON mn.id = pr.manufacturer_id
    JOIN prices pc ON pc.product_id = pr.id
GROUP BY mn.name
...
FROM products pr
    JOIN manufacturers mn ON mn.id = pr.manufacturer_id
    JOIN prices pc ON pc.product_id = pr.id
WHERE pc.value = (SELECT MAX(prc.value)
FROM prices prc)